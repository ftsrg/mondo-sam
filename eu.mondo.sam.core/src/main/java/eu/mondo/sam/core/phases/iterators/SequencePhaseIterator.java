...
public class SequencePhaseIterator implements Iterator<BenchmarkPhase> {
	...
	/**
	 * Equals with the current index of the phases list, which belongs to
	 * the SequencePhase object.
	 */
	private int index;
	...
	/**
	 * Refers to the iterator of the current BenchmarkPhase from the list,
	 * which belongs to the SequencePhase object.
	 */
	 ...
	 * 
	 * @param phase
	 *                SequencePhase object with initialized fields. It
	 *                mustn't contain an empty list of phases.
	 */
	...
	/**
	 * Returns an AtomicPhase object and increases the index variable. If
	 * this is invoked again after the last AtomicPhase was returned, starts
	 * over the iteration from the first element.
	 * 
	...
	public AtomicPhase next() {
		...
		if (iterator.hasNext() == false) {
			index++;
			if (index < size) {
				iterator = sequencePhase.getPhases().get(index)
						.iterator();
			}
		}
		return atomic;
	 ...
	 * 
	 * @return True, if the current index is lower than the size of list in
	 *         the SequencePhase object. In the other case, returns False.
	 */
	@Override
